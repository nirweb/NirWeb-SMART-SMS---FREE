...
//            $response = sa
        ...
        public function add_options_html() {
            ...
            echo wp_kses_post('<div class="csf-container">');
            ...
            echo wp_kses_post('<input type="hidden" class="csf-section-id" name="csf_transient[section]" value="1">');

            wp_nonce_field( 'csf_options_nonce', 'csf_options_nonce'. $this->unique );

            echo '<div class="csf-header'. esc_attr( $sticky_class ) .'">';
            echo wp_kses_post('<div class="csf-header-inner">');

            echo '<div class="csf-header-left">';
            echo wp_kses_post('<h1>'. $this->args['framework_title'] .'</h1>');
            echo wp_kses_post('</div>');

            echo wp_kses_post('<div class="csf-header-right">');

            $notice_class = ( ! empty( $this->notice ) ) ? 'csf-form-show' : '';
            $notice_text  = ( ! empty( $this->notice ) ) ? $this->notice : '';

            echo wp_kses_post('<div class="csf-form-result csf-form-success '. esc_attr( $notice_class ) .'">'. wp_kses_post($notice_text) .'</div>');

            echo ( $this->args['show_form_warning'] ) ? wp_kses_post('<div class="csf-form-result csf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'csf' ) .'</div>') : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? wp_kses_post('<div class="csf-expand-all" title="'. esc_html__( 'show all settings', 'csf' ) .'"><i class="fas fa-outdent"></i></div>') : '';
            ...
            echo wp_kses_post('<div class="csf-buttons">');
            ...
            echo wp_kses_post('<div class="csf-wrapper'. esc_attr( $show_all ) .'">');

            if ( $has_nav ) {

                echo wp_kses_post('<div class="csf-nav csf-nav-'. esc_attr( $nav_type ) .' csf-nav-options">');
                ...
                foreach ( $this->pre_tabs as $tab ) {
                    ...
                    if ( ! empty( $tab['subs'] ) ) {
                        ...
                        echo wp_kses_post('<li class="csf-tab-item">');

                        echo wp_kses_post('<a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'" class="csf-arrow">'. $tab_icon . $tab['title'] . $tab_error .'</a>');
                        ...
                        echo wp_kses_post('<li class="csf-tab-item"><a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'">'. $tab_icon . $tab['title'] . $tab_error .'</a></li>');
            ...
            echo wp_kses_post('<div class="csf-content">');

            echo wp_kses_post('<div class="csf-sections">');
            ...
            foreach ( $this->pre_sections as $section ) {
                ...
                echo wp_kses_post('<div class="csf-section hidden'. esc_attr( $section_onload . $section_class ) .'" data-section-id="'. esc_attr( $section_parent . $section_slug ) .'">');
                echo ( $has_nav ) ? wp_kses_post('<div class="csf-section-title"><h3>'. $section_icon . $section_title .'</h3></div>') : '';
                echo ( ! empty( $section['description'] ) ) ? wp_kses_post('<div class="csf-field csf-section-description">'. $section['description'] .'</div>') : '';
                ...
                if ( ! empty( $section['fields'] ) ) {
                    ...
                    echo wp_kses_post('<div class="csf-no-option">'. esc_html__( 'No data available.', 'csf' ) .'</div>');
            ...
            if ( ! empty( $this->args['show_footer'] ) ) {
                ...
                echo wp_kses_post('<div class="csf-footer">');
