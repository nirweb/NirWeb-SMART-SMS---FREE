...
if ( ! class_exists( 'CSF_Field_typography' ) ) {
  class CSF_Field_typography extends CSF_Fields {
    ...
    public function create_select( $options, $name, $placeholder = '', $is_multiple = false ) {
      ...
      $output  = '<select name="'. esc_attr( $this->field_name( '['. $name .']'. $multiple_name ) ) .'" class="csf--'. esc_attr( $name ) . esc_attr( $chosen_rtl ) .'" data-placeholder="'. esc_attr( $placeholder ) .'"'. esc_attr($multiple_attr) .'>';
    ...
    public function output() {
      ...
      if ( $font_family ) {
        $output .= 'font-family:"'. esc_attr($font_family) .'"'. esc_attr($backup_family) . esc_attr($important) .';';
      }
